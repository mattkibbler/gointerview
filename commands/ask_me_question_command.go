...
func (cmd *AskMeQuestionCommand) HandleInput(db *sql.DB, r *bufio.Reader) (Command, error) {
	...
	output.PrintBlock(output.PrintBlockOptions{Title: question.Question, Message: "Enter your answer..."})
	...
	if lastAnswer.GivenAnswer != "" {
		output.PrintBlock(output.PrintBlockOptions{Title: "You previously answered:", Message: lastAnswer.GivenAnswer})
		time.Sleep(time.Second)
	}
...
promptLoop:
	...
	if answerMarkedAsCorrect {
		output.PrintBlock(output.PrintBlockOptions{Title: "Well done!"})
	} else {
		output.PrintBlock(output.PrintBlockOptions{Title: "No worries, you'll do better next time..."})
	}
